...
export default class App extends Component {
  ...
  componentDidMount() {
    if (!navigator.onLine) {
      if (localStorage.getItem('peliculas') === null)
        ...
        this.setState({ peliculas: JSON.parse(localStorage.getItem('peliculas')) });
      }
      if (localStorage.getItem('pelicula') === null)
        this.setState({ seleccionada: {} })
      else {
        console.log(localStorage.getItem('pelicula'));
        this.setState({ seleccionada: JSON.parse(localStorage.getItem('pelicula')) });
      }
    }
    let urlEn = "https://gist.githubusercontent.com/josejbocanegra/8b436480129d2cb8d81196050d485c56/raw/48cc65480675bf8b144d89ecb8bcd663b05e1db0/data-en.json";
    let urlEs = "https://gist.githubusercontent.com/josejbocanegra/f784b189117d214578ac2358eb0a01d7/raw/2b22960c3f203bdf4fac44cc7e3849689218b8c0/data-es.json";
  ...
  setSelected(id) {
    ...
    localStorage.setItem('pelicula', JSON.stringify(this.state.peliculas[id - 1]));
  }
  getSelected() {
              ...
              <tbody>
                {this.state.peliculas.map((e, i) =>
                  <tr key={i} onClick={() => (this.setSelected(e.id))}>
                    ...
                    <td>{e.directedBy}</td>
                    <td>{e.country}</td>
                    <td>{e.budget} <FormattedPlural value={e.budget} one={<FormattedMessage id="singular" />} other={<FormattedMessage id="plural" />} /></td>
                    <td>
                      <FormattedDate
